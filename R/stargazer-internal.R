...
.stargazer.wrap <-
  function(..., type, title, style, summary, out, out.header, covariate.labels, column.labels, column.separate, 
           ...
           colnames,
           column.sep.width, decimal.mark, df, digit.separate, digit.separator, digits,p_digits,t_digits, digits.extra, 
           flip, float, 
  ...
  function(object.name, user.coef=NULL, user.se=NULL, user.t=NULL, user.p=NULL, auto.t=TRUE, auto.p=TRUE, user.ci.lb=NULL, user.ci.rb=NULL) {
    ...
    } else if (sum(grepl('regress', class(object.name))) > 0) {
      .summary.object <<- summary.regress(object.name)
    } else {
  ...
  function(part, which.variable, variable.name=NULL) {
  	...
  	# coefficients and stars
        ...
        # add my new function----p value 
        if ((.format.single.row == TRUE) & (("p-value" %in% .format.coefficient.table.parts) | ("p-value*" %in% .format.coefficient.table.parts))) {
            ...
            if (!is.na(.global.p.values[.global.coefficient.variables[which.variable],i])) {
              ...
              # report p-values
              cat(space.char, .format.p.values.left, .iround(.global.p.values[.global.coefficient.variables[which.variable],i],.format.pvalue.round.digits,round.up.positive=TRUE),.format.p.values.right,sep="")

              # add stars to denote statistical significance
              if ("p-value*" %in% .format.coefficient.table.parts) { 
                p.value <- .global.p.values[.global.coefficient.variables[which.variable],i]
                .enter.significance.stars(p.value) 
              }
            }
        ...
        # add new my function---- t statistics
        if ((.format.single.row == TRUE) & (("t-stat" %in% .format.coefficient.table.parts) | ("t-stat*" %in% .format.coefficient.table.parts))) {
            ...
            # report t-statistics
               cat(space.char, .format.t.stats.left, .iround(.global.t.stats[.global.coefficient.variables[which.variable],i],.format.tvalue.round.digits),.format.t.stats.right,sep="")

               # add stars to denote statistical significance
               if ("t-stat*" %in% .format.coefficient.table.parts) { 
                p.value <- .global.p.values[.global.coefficient.variables[which.variable],i]
                .enter.significance.stars(p.value) 
               }
            }
  	...
  	# p-values
  	else if (((part=="p-value") | (part=="p-value*")) & (.format.single.row==FALSE)) {
  		for (i in seq(1:length(.global.models))) {
  			if (!is.na(.global.p.values[.global.coefficient.variables[which.variable],i])) {

  				# report p-values
  				cat(" & ", .format.p.values.left, .iround(.global.p.values[.global.coefficient.variables[which.variable],i],.format.pvalue.round.digits,round.up.positive=TRUE),.format.p.values.right,sep="")
  	...
  	# t-statistics
  	else if (((part=="t-stat") | (part=="t-stat*")) & (.format.single.row==FALSE)) {
  		for (i in seq(1:length(.global.models))) {
  			if (!is.na(.global.t.stats[.global.coefficient.variables[which.variable],i])) {
  				# report t-statistics
  				cat(" & ", .format.t.stats.left, .iround(.global.t.stats[.global.coefficient.variables[which.variable],i],.format.tvalue.round.digits),.format.t.stats.right,sep="")
  ...
  function(object.name) {
    ...
    if (class(object.name)[1]=="regress") {
      return("regress")
    }
   ...
   if (!is.null(object.name$call)) {
  	...
  	else if (grepl("svyglm()",object.name$call[1])) {
  		if (object.name$family$family=="gaussian") {
  			if (object.name$family$link=="identity") {
  	...
  	else if (grepl("gam()",object.name$call[1])) {
  		if (object.name$family$family=="gaussian") {
  			if (object.name$family$link=="identity") {
  				return("normal.gam")
  			}
  	...
  	else if (grepl("polr()", object.name$call[1])) {
  	...
  	else if (grepl("gee()", object.name$call[1])) {
  		if (object.name$family$family=="gaussian") {
  			if (object.name$family$link=="identity") {
  				return("normal.gee")
  			}
  	...
  	else if (grepl("survreg()", object.name$call[1])) {
    ...
    if (grepl("coxph()", object.name$call[1])) {
  		return("coxph")
  	}
  	if (grepl("pmg()", object.name$call[1])) {
  	  return("pmg")
  	}
  	if (grepl("selection()", object.name$call[1])) {
  	  return("selection")
  	}
  	if (grepl("heckit()", object.name$call[1])) {
  	  return("heckit")
  	}
  	if (grepl("probit()", object.name$call[1])) {
  	  return("probit.ss")
  	}
  	if (grepl("binaryChoice()", object.name$call[1])) {
  	  return("binaryChoice")
  	}
  	if (grepl("brglm()", object.name$call[1])) {
  	  return("brglm")
  	}
  	if (grepl("gls()", object.name$call[1])) {
  	  return("gls")
  	}
  	if (grepl("clm()", object.name$call[1])) {
  	  return("clm")
  	}
  	if (grepl("lmrob()", object.name$call[1])) {
  	  return("lmrob")
  	}
    if (grepl("glmrob()", object.name$call[1])) {
       return("glmrob")
    }
  	if (grepl("dynlm()", object.name$call[1])) {
  	  return("dynlm")
  	}
  	if (grepl("rq()", object.name$call[1])) {
  	  return("rq")
  	}
  	if (grepl("gmm()", object.name$call[1])) {
  	  return("gmm")
  	}
  	if (grepl("lagsarlm()", object.name$call[1])) {
  	  return("lagsarlm")
  	}
  	if (grepl("errorsarlm()", object.name$call[1])) {
  	  return("errorsarlm")
  	}
  	if (grepl("rlm()", object.name$call[1])) {
  	  return("rlm")
  	}
  	if (grepl("aftreg()", object.name$call[1])) {
  	  return("aftreg")
  	}
  	if (grepl("coxreg()", object.name$call[1])) {
  	  return("coxreg")
  	}
  	if (grepl("phreg()", object.name$call[1])) {
  	  return("phreg")
  	}
  	if (grepl("weibreg", object.name$call[1])) {
  	  return("weibreg")
  	}
  	if (grepl("bj()", object.name$call[1])) {
  	  return("bj")
  	}
  	if (grepl("cph()", object.name$call[1])) {
  	  return("cph")
  	}
  	if (object.name$call[1]=="Gls()") {
  	  return("Gls")
  	}
  	if (grepl("lrm()", object.name$call[1])) {
  	  return("lrm")
  	}
  	if (grepl("ols()", object.name$call[1])) {
  	  return("ols")
  	}
  	if (grepl("psm()", object.name$call[1])) {
  	  return("psm")
  	}
  	if (grepl("Rq()", object.name$call[1])) {
  	  return("Rq")
  	}
  	if (grepl("hetglm()", object.name$call[1])) {
  	  return("hetglm")
  	}
    else if (grepl("relogit()", object.name$call[1])) {
      return("relogit")
    }
  	...
  	else if (grepl("tobit()", object.name$call[1])) {
  	  return("tobit(AER)")
  	}
    
    else if (grepl("multinom()", object.name$call[1])) {
      return("multinom")
    }
    
  	else if (grepl("betareg()", object.name$call[1])) {
  	  return("betareg")
  	}
  	else if (grepl("zeroinfl()", object.name$call[1])) {
  	  return("zeroinfl")
  	}
  	else if (grepl("hurdle()", object.name$call[1])) {
  	  return("hurdle")
  	}  	
  	else if (grepl("plm()", object.name$call[1])) {
  	  return("plm")
  	}
    else if (grepl("pgmm()", object.name$call[1])) {
       return("pgmm")
    }  	
  	else if (grepl("ivreg()", object.name$call[1])) {
  	  return("ivreg")
  	} 
   }
...
###########################################
    ...
    .format.pvalue.round.digits <- 3
    .format.tvalue.round.digits <- 2
    ...
    if (length(error.present) == 1) {
      ...
      ##################################################
      # add new digits choices for need 
      if (!is.null(p_digits)) {
        .format.pvalue.round.digits <- p_digits
      }

      if (!is.null(t_digits)) {
        .format.tvalue.round.digits <- t_digits
      }
